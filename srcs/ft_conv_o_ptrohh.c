...
/*   Updated: 2018/11/28 10:34:55 by erli             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int		max_width(const t_format *format, unsigned char *tab, int len)
{
	int			width;
	long long	pow;
	int			i;
...
static	int		make_list_tmp(t_format *format, unsigned char *tab, int len,
					t_list **list_tmp)
{
	t_format	*format_tab;
	t_format	*copy;
...
{
	unsigned char		*nb_tab;
	int					len;
	t_list				*list_tmp;

	nb_tab = va_arg(ap, unsigned char*);
	len = va_arg(ap, int);
	list_tmp = NULL;
